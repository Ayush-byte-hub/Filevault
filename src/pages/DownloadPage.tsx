      ...
      <div className="bg-white rounded-3xl border border-slate-200/80 p-6 sm:p-10 shadow-xs text-center">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 mb-3">
          ...
          {storageService.isCatboxUrl(file.fileUrl) ? (
            ...
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-50 text-blue-900 text-xs font-semibold border border-blue-200/70">
              <HardDrive className="w-3.5 h-3.5 text-blue-600" />
              <span>Workers KV Vault</span>
            </div>
          ) : null}
        </div>
            ...
            <div className="space-y-4">
              ...
              {storageService.isCatboxUrl(file.fileUrl) && (
                <button
                  type="button"
                  onClick={() => {
                    storageService.triggerFileDownload(file, true);
                    showToast('Direct Mirror Triggered', 'Opening direct Catbox.moe download stream', 'info');
                  }}
                  className="w-full py-2 px-4 text-xs font-semibold text-slate-600 hover:text-slate-900 bg-white hover:bg-slate-100 border border-slate-200 rounded-xl transition-colors flex items-center justify-center gap-1.5"
                >
                  <ExternalLink className="w-3.5 h-3.5 text-slate-400" />
                  <span>Direct Catbox.moe Mirror</span>
                </button>
              )}
              ...
              <p className="text-[11px] text-slate-400">
                {storageService.isCatboxUrl(file.fileUrl)
                  ...
                  : 'Fast direct transfer. Safe, unthrottled, and free of bundled software.'}
              </p>
            </div>
          )}
        </div>
